...
export default {
  ...
  setup() {
    ...
    return {
      // getter
      fullname: computed(() => store.state[module].token.fullname),

      token: computed(() => store.state[module].token.name),
      path: computed(() => store.state[module].token.path),
  ...
  computed: {
    ...mapState({
      getNumberToken: (state) => state[module].token.amount,
    }),
  },
  async mounted() {},
  methods: {
